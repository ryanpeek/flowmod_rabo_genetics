...
norm.s<-function(x) {x/sum(x)} # function to normalize

site<-"RABO_eel_100k" #ALAME, MFA, NFA, RUBIC, NFMFA

# read data
s<-scan(paste0('data_output/sfs/', site, '.sfs'))

length(s)
s <- s[2:length(s)]
# the variable categories of the sfs & normalize
sfs<-norm.s(s[-c(1,length(s))])

# the variability as percentile
pvar<- (1-sfs[1]-sfs[length(sfs)])*100

# plot
barplot(sfs, xlab="Chromosomes (with Derived Allele)", names=1:length(sfs), ylab="Proportion of Variable Sites", legend=paste("Variability:= ",round(pvar,3),"%"), main=paste0(site, " SFS"),col='blue') # plot the SFS (for variable categories only...unfolded)
