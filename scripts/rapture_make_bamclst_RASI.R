...
# specific PLATE
# dat<- filter(metadat, SPP_ID=="RABO") %>% filter(RAD_ID=="RAD-209")
# dat<- filter(metadat, SPP_ID=="RABO") %>% filter(RAD_ID=="RAD-213" | RAD_ID=="RAD-216"| RAD_ID=="RAD-219")
# dat<- filter(metadat, SPP_ID=="RABO") %>% filter(RAD_ID=="RAD-209" | RAD_ID=="RAD-210" | RAD_ID=="RAD-211" )
# dat<- filter(metadat, SPP_ID=="RABO") %>% filter(RAD_ID=="RAD-205" | RAD_ID=="RAD-206" | RAD_ID=="RAD-207" | RAD_ID=="RAD-208")

# MUSSULMAN SAMPLES ONLY:
#dat<- filter(metadat, RAD_ID=="RAD-212" | RAD_ID=="RAD-213") %>% filter(is.na(YYYY))

# ALL RANIDS (WSU4 Plate):
#dat<- filter(metadat, Locality=="WESTUS")

# add 2 RABO from ROBBERS and NFA mainstem, two RASI from Fordyce
dat<- filter(metadat, Locality=="WESTUS" | grepl('RAP603|RAP606|RAP773|RAP774|SM036|SM030', SampleID))
...
# write to csv:
summary(dfout)
#dfout <- dfout %>% select(Seq:Locality_details)
#write_csv(dfout, "data_output/rasi_all_50k_metadata.csv")
...
paste0("sbatch -p high —mem=64G -t 24:00:00 --mail-type ALL --mail-user rapeek@ucdavis.edu 03a_pca_new.sh bamlist_mrg_",site,"_",bamNo,"k_thresh", " ", lsite, "_",bamNo,"k_thresh", " bamlists/POPS")
